...
const CareRequest = () => {
    ...
    const { data: careReq = [] , refetch} = useQuery({
        queryKey: ["careReq"],
        queryFn: async () => {
          const res = await axiosSecure.get(`/requestForCare?${collectUser?.email}`);
      ...
      const handleVerify = (item) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axiosSecure
              .patch(`/acceptedCareClient/${item._id}`, { careStatus: "accepted" }) // Corrected URL
              .then((res) => {
      ...
      const handleReject = (item) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axiosSecure
              .patch(`/acceptedCareClient/${item._id}`, { careStatus: "rejected" }) // Send the status in the request body
              .then((res) => {
                if (res.data.modifiedCount > 0) {
                  Swal.fire({
                    title: "Successful",
